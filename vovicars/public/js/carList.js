...
var aracMarka = document.querySelector(".aracMarka");
var aracModel = document.querySelector(".aracModel");
var aracSeri = document.querySelector(".aracSeri");

// Populate the select box with car brands
cars.forEach(function(brand) {
    var option = document.createElement("option");
    option.text = brand.brand;
    option.value = brand.brand;
    aracMarka.add(option);
});

var model;
var seri;

aracMarka.addEventListener("change", () => {
    aracModel.innerHTML="";
    for (let i = 0 ; i<cars.length; i++)
    {
        if (cars[i].brand==aracMarka.value)
        {
            model = cars[i].models;
        }
    }
    if (model!=null)
    {
        model.forEach(function(m) {
            ...
            aracModel.add(option);
        });
    }
})

aracModel.addEventListener("change", () => {
    aracSeri.innerHTML="";
    for (let i = 0 ; i<cars.length; i++)
    {
        if (cars[i].brand==aracMarka.value)
        {
            ...
            {
                if (cars[i].models[j].title==aracModel.value)
                {
                    seri = cars[i].models[j].types;
                }
            }
        }
    }
    if (seri!=null)
    {
        seri.forEach(function(m) {
            var option2 = document.createElement("option");
            option2.text = m;
            option2.value = m;
            aracSeri.add(option2);
        });
    }
})
